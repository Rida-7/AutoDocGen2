...
async def execute_workflow(user_id: str, project_id: str, data: dict = None, db=None):
    ...
    if not token:
        print("❌ User not connected to Trello")
        return {"status": "error", "message": "User not connected to Trello"}
    ...
    input_state = WorkflowState(
        ...
        pm_data={},  # Trello cards will be fetched by workflow nodes
    ...
    existing_entry = await docs_collection.find_one({
        "user_id": user_id,
        "project_id": project_id,
    ...
    if existing_entry:
        existing_doc = existing_entry.get("generated_docs", "")
