...
async def register_trello_webhook(
    *,
    ...
    """
    Registers a Trello webhook for a board, with exponential backoff on 502/504 errors.
    """
    url = "https://api.trello.com/1/webhooks"
    data = {"idModel": board_id, "callbackURL": callback_url, "description": "AutoDocGen Webhook"}
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                if e.response.status_code in [502, 504]:
                    ...
                    await asyncio.sleep(delay)
                    continue
                print(f"❌ Trello API Error: {e.response.text}")
                return e.response
    ...
    print(f"❌ Failed to register webhook after {retries} attempts for board {board_id}")
    return None
